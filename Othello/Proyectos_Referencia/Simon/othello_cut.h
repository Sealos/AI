...
#define DIM           36
#define N             6
#define POS(i, j, more) (N*i + j + more)
...
class state_t {
  ...
  public:
    ...
    std::vector<state_t> get_valid_states(bool color) {
        std::vector<state_t> valid_states;
        for( int pos = 0; pos < DIM; ++pos ) {
            if( (color && is_black_move(pos)) || (!color && is_white_move(pos)) ) {
            	state_t s(*this);
            	s = s.move(color,pos);
...
inline int get_pos(int i, int j) {
  if ((i <= 1) || (i == 2 && j <= 1)) {
    // caso facil estamos por encima del centro
    return POS(i, j, 4);
  } else if ((i == 2 && j >= 4 && j <= 5) ||
	     (i == 3 && j >= 0 && j <= 1)) {
    return POS(i, j, 2);
...
inline void state_t::blank(int pos) {
  if (pos >= 4)  {
    free_ &= ~(1 << pos-4);
    pos_ &= ~(1 << pos-4);
  }
...
void state_t::rotate90() {
  for (int x=0; x<(N/2); x++) {
    for(int y=0; y<(N/2); y++) {

      int pos1 = get_pos(x, y), pos2 = get_pos(y, N-1-x), 
	pos3 = get_pos(N-1-x, N-1-y), pos4 = get_pos(N-1-y, x);
      bool tmp = is_black(pos1), free_tmp = is_free(pos1);
      if (is_free(pos2)) 
	blank(pos1);
      else
	set_color(is_black(pos2), pos1);
      
      if (is_free(pos3))
	blank(pos2);
      else
	set_color(is_black(pos3), pos2);
      
      if (is_free(pos4))
	blank(pos3); 
      else
	set_color(is_black(pos4), pos3);
